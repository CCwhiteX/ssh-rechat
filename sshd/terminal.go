...
import (
	...
	"github.com/ccwhitex/ssh-rechat/sshd/terminal"
	...
	go func() {
		...
		for {
			...
			case <-ticker:
				...
				if err != nil {
					// Connection is gone
					logger.Error("Keepalive failed, closing terminal", "addr", term.Conn.RemoteAddr(), "error", err)
					term.Close()
					return
				}
			case <-term.done:
...
func NewSession(conn *ssh.ServerConn, channels <-chan ssh.NewChannel) (*Terminal, error) {
	...
	for ch := range channels {
		if t := ch.ChannelType(); t != "session" {
			logger.Error("Ignored channel type", "addr", conn.RemoteAddr(), "channel_type", t)
...
func (t *Terminal) Close() error {
	...
	t.closeOnce.Do(func() {
		...
		if err := t.Channel.Close(); err != nil {
			logger.Error("Failed to close terminal channel", "addr", t.Conn.RemoteAddr(), "error", err)
		}
		err = t.Conn.Close()
	})
	return err
