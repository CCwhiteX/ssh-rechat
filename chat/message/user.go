	...
	u.closeOnce.Do(func() {
		if u.screen != nil {
			if err := u.screen.Close(); err != nil {
				logger.Error("Failed to close user screen", "user", u.ID(), "error", err)
			}
		}
...
func (u *User) writeMsg(m Message) error {
	...
	if err != nil {
		logger.Error("Write failed, closing user", "user", u.ID(), "error", err)
		u.Close()
	}
	return err
}
...
func (u *User) Send(m Message) error {
	...
	case <-time.After(messageTimeout):
		logger.Error("Message buffer full, closing user", "user", u.ID())
		u.Close()
		return ErrUserClosed
	}
	return nil
