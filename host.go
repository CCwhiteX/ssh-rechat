...
func (h *Host) Connect(term *sshd.Terminal) {
	...
	if err != nil {
		logger.Error("[%s] Failed to join: %s", term.Conn.RemoteAddr(), err)
		return
	}
	...
	logger.Debug("[%s] Joined: %s", term.Conn.RemoteAddr(), user.Name())
	...
	for {
		...
		if err == io.EOF {
			...
			logger.Error("[%s] Terminal reading error: %s", term.Conn.RemoteAddr(), err)
			break
		}
	...
	err = h.Leave(user)
	if err != nil {
		logger.Error("[%s] Failed to leave: %s", term.Conn.RemoteAddr(), err)
		return
	}
	logger.Debug("[%s] Leaving: %s", term.Conn.RemoteAddr(), user.Name())
}
